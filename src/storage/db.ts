...
export const db = {
  ...
  fetchServerSlots: async (): Promise<Slot[]> => {
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          if (data.length > 0) {
            ...
            // Check if local has slots that need to be uploaded to server
            const raw = localStorage.getItem(STORAGE_KEYS.SLOTS);
            if (raw) {
              try {
                const local = JSON.parse(raw);
                if (Array.isArray(local) && local.length > 0) {
                  db.syncSlotsToServer(local);
                  return local;
                }
              } catch {}
            }
      ...
      console.warn('Fetch server slots fallback:', e);
    }
    return db.getSlots();
  },
  ...
  fetchServerAppointments: async (): Promise<Appointment[]> => {
    try {
        ...
        if (Array.isArray(data)) {
          const mapped: Appointment[] = data.map((item: any) => ({
            ...
            paciente: {
              ...
              sus: item.pacienteSus || '',
              tel: item.pacienteTelefone || '',
              nascido: item.pacienteDataNasc || '',
              mae: item.pacienteMae || '',
              endereco: item.pacienteEndereco || '',
              cep: item.pacienteCep || '',
            },
            ...
            operadorTelefone: item.operadorTelefone || undefined,
            data: item.data,
            horario: item.horario,
            especialidade: item.especialidade,
            medico: item.medico || undefined,
            motivoCancelamento: item.observacoes || undefined,
      ...
      console.warn('Fetch server appointments fallback:', e);
    }
  ...
  saveSlots: (slots: Slot[]) => {
    ...
    fetch('/api/slots/replace-all', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ slots }),
    }).catch(e => console.warn('Sync slots replace-all error:', e));
  },
  ...
  saveAppointments: (appointments: Appointment[]) => {
    ...
    }).catch(e => console.warn('Sync appointments replace-all error:', e));
  },
