...
export default function App() {
  ...
  const handleAddSlots = async (newSlots: Slot[]) => {
    const result = await db.createSlotsApi(newSlots);
    if (!result.success) {
      alert(result.error || 'Não foi possível criar as vagas no banco central.');
      return;
    }
    const fresh = await db.fetchServerSlots();
    setSlots(fresh || []);
  };
  ...
  const handleSaveAppointment = async (newApp: Appointment): Promise<{ success: boolean; error?: string }> => {
    // V2: the server/database performs the reservation atomically.
    const result = await db.reserveAppointmentApi(newApp);
    if (!result.success) {
      // Re-read the central state so this browser immediately reflects the winner.
      const [freshSlots, freshApps] = await Promise.all([
        db.fetchServerSlots(),
    ...
    // Confirmed by central DB: refresh authoritative data instead of merging a local clone.
    const [freshSlots, freshApps] = await Promise.all([
      db.fetchServerSlots(),
      db.fetchServerAppointments(),
    ]);
  ...
  const handleRequestCancel = async (appointmentId: string, motivo: string) => {
    const result = await db.requestCancelAppointmentApiV2(appointmentId, motivo);
    if (!result.success) {
      alert(result.error || 'Não foi possível solicitar o cancelamento.');
      return;
    }
    setAppointments(await db.fetchServerAppointments());
  };

  const handleApproveCancel = async (appointmentId: string) => {
    const targetApp = appointments.find(a => a.id === appointmentId);
    if (!targetApp) return;
    const result = await db.cancelAppointmentApiV2(appointmentId, targetApp.motivoCancelamento || 'Cancelamento aprovado pelo Administrador Master');
    if (!result.success) {
      alert(result.error || 'Não foi possível cancelar o agendamento.');
      return;
    }
    const [freshSlots, freshApps] = await Promise.all([db.fetchServerSlots(), db.fetchServerAppointments()]);
    setSlots(freshSlots || []);
    setAppointments(freshApps || []);
  };

  const handleRejectCancel = async (appointmentId: string) => {
    const result = await db.rejectCancelAppointmentApiV2(appointmentId);
    if (!result.success) {
      alert(result.error || 'Não foi possível rejeitar o cancelamento.');
      return;
    }
    setAppointments(await db.fetchServerAppointments());
  };
